...
function App() {
    ...
    useEffect(() => {
        if (autoTrackEnabled && trackingStartDate && dietState && loaded) {
            ...
            const currentWeekNum = getWeeksSinceStart(trackingStartDate);
            
            const days = Object.keys(dietState.piano_alimentare[`settimana_${currentWeekNum}`]);
    ...
    }, [autoTrackEnabled, trackingStartDate, dietState, loaded]);
    ...
    const enableAutoTracking = () => {
        ...
        if (dietState) {
            const currentDay = getDayOfWeek();
            const days = Object.keys(dietState.piano_alimentare[`settimana_1`]);
            const dayIdx = days.indexOf(currentDay);
            if (dayIdx !== -1) {
                setCurrentWeek(1);
                setCurrentDayIndex(dayIdx);
    ...
    const isCurrentDay = () => {
        ...
        return currentWeekNum === currentWeek && currentDay === currentDayKey;
    };
